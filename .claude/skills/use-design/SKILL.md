...
## 14. 🔒 Переход на кастомный дизайн — процесс, а не переключатель

Блоками собирается почти всё, и почти всегда это верный ответ. Но у блочной сборки есть честная
граница: страница остаётся **внутри отведённого ей layout**. Выйти за него, положить холст во всю
ширину экрана, отдать страницу целиком внешнему дизайн-навыку — этого блоками не сделать.

Для таких случаев есть **свободный слой** `app/[lang]/(freeLayer)/`: голый `<body>` и твоя разметка.

### Что происходит физически

Языковое дерево расщеплено на два слоя, и группы в адресе не видны:

```
app/[lang]/
  layout.tsx        ← <html lang>, метаданные, манифест, шрифт, тема — остаётся ВСЕМ
  (designLayer)/    ← меню, подвал, баннер согласия, тосты, PWA — умолчание
  (freeLayer)/      ← ничего; страница рисует что угодно
```

### Порядок, и он не сокращается

1. **Спроси, нужен ли кастом вообще.** Часто «хочу не как у всех» решается своей палитрой, своим
   шрифтом и парой виджетов — без выхода из слоя оформления.
2. **Проверь разрешение** — `customDesign` в `DESIGN-CONFIG`, умолчание `false`. Выключено — скажи,
   где включается (панель → «Дизайн»), и остановись. Обходить нечего.
3. **Назови цену вслух и до работы** (таблица ниже).
4. **Переноси по одной странице.** Весь сайт разом — это ручная отладка всего сайта; страница за
   страницей — отладка одной.
5. **Запиши перенос** в `CLAUDE.md`, раздел «Страницы на кастомном дизайне»: адрес, дата, что снято.

### Цена, названная поимённо

| Что теряет страница | Следствие |
|---|---|
| палитру из панели | владелец меняет цвет в конструкторе — эта страница не меняется |
| меню и подвал | навигации нет; если нужна — рисуешь свою |
| баннер согласия | согласие на этой странице не собирается |
| установку приложения и воркер | ни предложения установки, ни офлайна |
| наши гейты вида | ни типографика, ни контраст, ни лимиты её больше не проверяют |

🔒 **Дальше эту страницу ведёт архитектор руками — это его зона ответственности, а не платформы**
(слова владельца, 2026-08-22). Сказать это до переноса обязательно: человек имеет право знать, что
берёт работу на себя.

### Чего в этом слое НЕ делают

- Не тащат туда блог, каталог, правовые страницы и всё за ролью: там ценность и есть структура.
- Не переносят страницу «на время, потом вернём» — возврат стоит столько же, сколько переход.
- Не считают свободный слой местом для быстрых экспериментов: он не песочница, а решение владельца.

## 15. Если просьба спорит с этим документом
