...
## 🔒 Дерево маршрутов растёт из СУЩНОСТЕЙ и РОЛЕЙ, а не из списка экранов

✗ **Оплачено 2026-08-24 на живой миграции.** Агент перенёс дерево, взяв список экранов исходника и
разложив его по группам прав. Получилось работающе и неверно: одна и та же сущность оказалась в двух
маршрутах группы персонала, личный кабинет получил один адрес без разбора, кому он принадлежит, а
действие над сущностью стало отдельной страницей. Конфликт всплыл бы на рефакторинге, когда переехало
бы уже многое.

**Образец стоит в этом же репозитории и отвечает на всё:** `products` — ОДНА сущность в ЧЕТЫРЁХ
группах (`shopping` · `manage` · `administration` · `accounting`), и `[productId]` есть только там,
где открывают конкретный экземпляр.

### Матрица «сущность × роль» строится ДО дерева

| Что определяешь | Вопрос |
|---|---|
| сущности | какие СУЩНОСТИ есть в проекте — не экраны, а вещи, у которых есть таблица и жизнь |
| касание | какая роль касается сущности и в каком режиме: **список** · **конкретный экземпляр** · **только свой экземпляр** |
| динамика | `[id]` появляется ровно там, где открывают **конкретный экземпляр, который не твой**. «Свой» берётся из сессии и сегмента не требует |
| группировка | одна сущность, к которой обращаются несколько ролей, даёт **несколько поверхностей одной сущности**, а не несколько разных маршрутов |
| страница или действие | новое умение — это **отдельная страница** или **действие на уже существующей**? Назначить признак, сменить статус, пригласить — почти всегда действие |

🔒 **Последний вопрос — самый дешёвый и самый пропускаемый.** Отдельная страница ради одного действия
над чужой сущностью создаёт второй экран того же назначения, и оба потом расходятся.

🔒 **Личный кабинет — не один маршрут по умолчанию.** Сначала ответь: кто ЕЩЁ его открывает. Никто —
адрес без сегмента, данные из сессии. Открывает персонал — появляется вторая поверхность с `[id]`, и
это не тот же маршрут с флагом, а другая группа прав.

**Проверка перед тем, как рисовать дерево:** каждая сущность названа · у каждой перечислены роли и
режимы · каждый `[id]` объяснён тем, что открывают чужой экземпляр · ни одна сущность не встречается
дважды в одной группе прав.
