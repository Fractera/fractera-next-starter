...
## 🔒 Пятая лгущая проверка: поиск по телу ответа ловит МЕБЕЛЬ

✗ **Оплачено живьём 2026-08-24.** Проверка звучала безупречно: «страница содержит свой заголовок» —
`curl` и поиск строки в ответе. Все страницы прошли. Негативный контроль убил её за секунду: на
`/tasks` находился и заголовок «Данные компании» — **ящик навигации печатает подписи ВСЕХ страниц на
каждой**.

🔒 **Там, где на странице есть меню, оглавление, хлебные крошки или подвал со ссылками, поиск по телу
подтверждает существование НАВИГАЦИИ, а не содержания.** Верная проверка адресуется тому элементу,
который принадлежит именно этой странице: разбор `<h1>`, `<title>`, конкретного узла.

**Как поймать это у себя:** та же проверка, применённая к ЧУЖОЙ странице, обязана дать другой ответ.
Даёт тот же — вы измеряете мебель.

**Признак того же дефекта в других формах:** «в HTML есть слово X» · «в бандле встречается имя
функции» · «в логе есть строка» — всё это про присутствие текста где-то, а не про работу способности.
