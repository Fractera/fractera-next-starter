...
description: >
  ...
  on /service/ai-draft-settings. Use when you notice an automation opportunity, a repeating
...
The draft lands on `/service/ai-draft-settings` for the architect to review. You do not
...
The page `/service/ai-draft-settings` is backed by a plain HTTP API on the local
admin service (`:3002`; the ai-draft-settings routes moved out of the slot into the
admin app in step 170, their filesystem roots still resolve to the slot via
`slotRoot()`). Create the draft, then fill its source + tasks. Always send the
`X-Agent-Identity` header so the action is attributed to you.

**Step 1 — create the draft record:**
```bash
curl -s -X POST http://127.0.0.1:3002/api/ai-draft-settings \
...
```bash
curl -s -X PATCH http://127.0.0.1:3002/api/ai-draft-settings/<id> \
  -H "Content-Type: application/json" \
  -H "X-Agent-Identity: <your-agent-id>" \
...
> Draft «<name>» published to /service/ai-draft-settings → <agent> → Skills (or MCP).
