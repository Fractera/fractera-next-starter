...
- **Платформу клиент не меняет — механизма нет.** Развёртывание делает `rm -rf /opt/fractera` и
  клонирует её заново с адреса Fractera; адрес зашит в `bootstrap.sh`, параметра под свой репозиторий
  нет — в отличие от слота. Правка на сервере не переживает следующего развёртывания и не хранится
  нигде. Лицензия правку разрешает; отсутствует именно механизм её сохранения.
- **Нужна правка платформы — это запрос в Fractera:** `admin@fractera.ai`. В письме назвать: что
  понадобилось и зачем, какая служба и порт, файл и строка, что вернул вызов и чего ждали, чем
  обошлись пока. Запрос такого состава — техническое задание; «у меня не получилось» — нет.
...
механическая работа, и отвлекаться на неё раньше незачем.
