...
- **Менять платформу нельзя, и это не запрет, а отсутствие места.** Развёртывание делает
  `rm -rf /opt/fractera` и ставит её заново: правка на сервере не переживает следующего развёртывания
  и не хранится нигде.
- **Нужна правка платформы — панель, раздел «Как построить этот проект».** В конце страницы врезка
  «Изменения в самой платформе»: кнопка выдаёт готовый текст письма с адресом этого сервера, владелец
  отправляет его со своей почты. Твоя часть — назвать службу, файл и строку и сказать, есть ли обход.
...
**🔒 Режим решает ровно одно — обязательны ли кейс и шаг.** Технические законы действуют во всех трёх:
статика публичных страниц, лимиты, переводы вместо вписанного текста, настройки из конфигов. Закон,
который режим мог бы выключить, законом не был бы, и «классический» никогда не означает «разрешено
сломать».

**Цена — часть выбора, и она за владельцем.** Кейсы дороже шагов, шаги дороже классического. К тому,
какая модель тебя исполняет, режим не привязан: проверить это изнутри нельзя, а непроверяемый запрет
обходится молча.
