...
- **Дизайн.** Как он задаётся проекту — `DESIGN-CONFIG`, примитивы, типографика — нет ни строки.
- **Секции.** Как создаются и правятся секции страницы. Сама структура секций ещё требует доделки,
  поэтому текст пишем после неё, а не до.
...
- **Панель ещё не выдаёт доступ к серверу.** Скрипты `scripts/server/` читают `FRACTERA_SSH_*` из
  `.env.local`, а выгрузка окружения (`/api/config/env-export`) кладёт туда только `REMOTE_DATA_URL`,
  `DATA_SECRET` и переменные приложения. Пока панель не научится выпускать пару ключей и вписывать её
  в выгрузку, эти четыре переменные появляются в проекте только руками владельца. Это следующая
  работа по платформе, а не свойство продукта.
