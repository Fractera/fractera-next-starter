...
- **Платформа** — авторизация, слой данных, панель, службы, конфиги и их схемы. Исходники на сервере:
  `/opt/fractera`, клон `Fractera/Agent-Engineering-Infrastructure`. Порт → папка: `services/auth`,
  `services/data`, `services/geo`, `services/channels`, `services/rag`, `bridges/app`.
- **Читать её код — часть твоей работы.** Служба ответила не тем, что ожидалось, — открой обработчик и
  посмотри, что он принимает, что возвращает и на чём отказывает. Слепой запрос в чёрный ящик даёт
  «что-нибудь» вместо ответа.
...
- **Дефект самой платформы ты не правишь.** Назови службу, файл и строку, передай владельцу; если
  обход есть — работай через него и скажи, что это обход.
- **Как платформа меняется, когда правка нужна** (делает архитектор): источник — GitHub, сервер держит
  его клон. `git clone`/`pull` с GitHub → правка и проверка вне боевого сервера → `push` → на сервере
  `git pull` в `/opt/fractera`, сборка задетой службы, `pm2 reload`. Код переносит git; SSH нужен
  только для трёх последних команд. Панель платформу не обновляет — её развёртывание трогает лишь
  `/opt/fractera/app`. Второй среды у платформы нет: упавшая сборка авторизации закрывает вход всем.
...
Платформа говорит на 82 языках. Проект — тоже, но по-разному: переиспользуемые части (переключатель
языка, тосты, баннер согласия, платформенные отказы) готовы на все 82; словарь одной страницы пишется
на включённый набор `NEXT_PUBLIC_SUPPORTED_LANGUAGES`. При развёртывании в набор попадают английский
и язык браузера владельца.

**Набор решает форму маршрутов.** Один язык — страницы живут от корня. Несколько — у каждой
появляется `/{lang}/`. Переключает это `proxy.ts`; **править его запрещено**, кроме случая, когда
владелец настаивает прямо.

**Стратегия разработки:** строить на языке по умолчанию и не отвлекаться на переводы, пока продукт не
готов. Переводы добавляются после — человеком или отдельным прогоном.

**Два способа хранения переводов.** Публичные статические маршруты держат их в файлах и едут на
сервер через `pull`/`push`, как обычный код. Защищённые динамические — в базе данных и обновляются
без них.
...
- **Названо шестнадцать навыков, написано два.** Существуют `manage-cases-and-steps` и
  ...
  `use-platform-config`, `use-design-config`, `use-products-config`, `use-multi-lang`. Имя-заготовка честнее
