...
Ключ — файл, не пароль: `ssh -i` работает без терминала и без обёрток. `.env.local` и папка ключа
лежат в `.gitignore` — в репозиторий они не уезжают никогда. Переменных нет — сервер для тебя закрыт:
скажи владельцу одной фразой, что нужна выгрузка окружения из панели, и работай через GitHub, пока он
её не даст. Не выдумывай адрес и не проси пароль в переписке.

**Что ты делаешь на сервере.** Проект живёт в `/opt/fractera/app`, и это единственная папка, которую
ты трогаешь:

- `git pull` в слоте — привезти отправленное;
...
- `curl -s localhost:3000/api/health`, чтение `.env.local` и `DEPLOYED_COMMIT` — что сейчас живо.

**Как доказывается, что доехало.** Двумя пруфами из разных плоскостей, и «собралось» — не пруф.
Первый: код выхода `npm` равен нулю и в логе твой маркер. Второй: после `reload` порт держит процесс
pm2 или его прямой потомок, а `uptime` растёт между двумя замерами. Разошлись — на порту сирота от
прошлой сессии, она отвечает HTML своей старой сборки, файлов с теми хэшами уже нет, и владелец видит
белый экран. Лечится по порядку: `pm2 stop` → `fuser -k -n tcp 3000` → порт пуст → `pm2 start`.

**Сборка упала — сначала причина, потом повтор.** Прочитай лог, назови строку, впиши симптом, причину
и лечение в `development-docs/ANTI-PATTERNS.md` и только потом собирай снова. Документ уезжает с твоим
`push` и потому переживает следующее развёртывание; память сессии — нет.
