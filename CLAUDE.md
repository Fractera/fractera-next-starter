...
## 🪦 Два слоя оформления — ОТКАЧЕНО 2026-08-22, в тот же день

Здесь стояло описание расщепления языкового дерева на `(designLayer)` и `(freeLayer)` с рубильником
`customDesign`. Построено и откачено по решению владельца в день постройки.

**Почему откачено.** Два комплекта страниц в одном дереве он назвал бардаком, и это подтвердилось делом:
перенос ослепил восемь гейтов, оставив их зелёными; карте сайта понадобился ручной заслон; в инструкции
начали появляться условия «если страница в кастомном слое, то…». Ветвление платится в каждом будущем
гейте и каждом навыке, и платится молча.

**Что вместо него.** Обсуждается раздел «Архитектура» в панели: не два слоя внутри приложения, а выбор
архитектуры целиком — стандартная Fractera, свободное проектирование, параллельная маршрутизация. Пока
решение не принято, кастомного слоя в проекте НЕТ, и обещать его нельзя.
