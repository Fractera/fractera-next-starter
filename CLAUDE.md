...
### 6.1. Обращение пользователя · brainstorm
Уточни, хочет ли архитектор брейншторм. Веди опрос, пока не кончатся вопросы или пока он не скажет «go».
Опционально, если запрос об этом:
- **6.1.1** Обработать черновики страницы `/ai-draft-settings` (папка `AI-DRAFT-SETTINGS/<АГЕНТ>/`, подпапки
  `SKILLS/`, `MCP/`): поочерёдно генерировать навыки/коннекторы (MCP),
  каждый — с согласованием. Извлечённый черновик переносится в новый шаг как ТЗ, запись-черновик удаляется
  (Discard / Remove draft); оригиналы инструкций при этом не трогаются.
- **6.1.2** «Завершить страницу X»: открыть `/architecture`, найти незавершённые (declared) узлы — у них
  есть запись в `README.md` маршрута («читай меня»). Перейти к 6.2.

### 6.2. Обогащение вводной информации
- **6.2.1** Изучить страницу `/development-steps` (папка `DEVELOPMENT-STEPS/`): новые шаги в `NEW-STEPS/`; часть
  может быть не завершена.
- **6.2.2** Точечные запросы к вектор-памяти (`POST /api/rag/query`) для расширения контекста — по задаче,
  не дамп.
- **6.2.3** Беглый, поверхностный просмотр страницы `/patterns` (папка `PATTERNS/`): запомнить **имена**
  существующих паттернов —
  пригодятся для переиспользования.

### 6.3. Development step — открытие шага
- **6.3.1** Общее описание шага: входные данные, планируемый результат, промежуточные результаты
  (декомпозиция), планируемые изменения в дереве маршрутизации.
- **6.3.2** Создать промежуточные под-шаги, очевидные на текущем этапе. Доращивать новые под-шаги в
  следующих циклах — допускается и нормально.
- **6.3.3** При необходимости внести изменения в дерево `/architecture`. Маршрут описывается двумя
  файлами: `_meta.ts` (дескриптор `RouteMeta`, по стандарту типов) и `README.md` (живой to-do). Объявить
  страницу/эндпоинт = создать `README.md` (declared-узел); задачи маршрута пишутся через
  `/api/project/default/architecture/tasks`. На этом этапе ты можешь и должен, если нужно, менять дерево.

### 6.4. Цикл разработки (повторяемый)
- **6.4.1** Извлечь паттерны, которые могут понадобиться в этом цикле (`/patterns`; нет подходящего —
  создать новый и согласовать).
- **6.4.2** Взять из дерева `/architecture` задачу соответствующего маршрута в разработку — то есть
  получить существующие шаги и другие элементы по этому узлу. Выполнить очередной под-шаг:
  - **6.4.2.1** Завершить под-шаг: в дереве `/architecture` закрыть задачу, очистить `README.md`
    (declared→live, когда появился реальный файл маршрута).
  - **6.4.2.2** Декомпозировать задачу на подзадачи: добавить новые под-шаги в `/development-steps` и
    новые to-do-записи в дерево `/architecture`.

Повторяй цикл столько раз, сколько необходимо. **Каждую итерацию отмечай в чек-листе** задачи.

### 6.5. Тестирование
Запустить тестирование, проверить выполнение задачи.

### 6.6. Два доказательства
Представить архитектору **два доказательства из разных областей/плоскостей**, что задача выполнена в полной
мере (например: страница рендерит результат **и** строка в БД появилась).

### 6.7. Подготовка развёртывания (только по разрешению архитектора)
- **6.7.1** Загрузить и **перечитать анти-паттерны** (`PATTERNS/ANTI-PATTERNS/`), связанные с
  развёртыванием, — ещё раз проверить перед запуском.
- **6.7.2** Если обнаружено расхождение — отменить развёртывание, исправить ошибку.
- **6.7.3** Запустить развёртывание (`POST /api/deploy` → async build → `pm2 reload`). Дождаться
  результата.

### 6.8. Результат развёртывания
- **6.8.1 Провал.** Сделать запись в таблице Deployments (`status=error`, отметить коммит и неудачу).
  Изучить ошибки. Добавить новый **анти-паттерн** об ошибке развёртывания. Исправить — повторить
  развёртывание.
- **6.8.2 Успех.**
  - **6.8.2.1** Изучить, какие паттерны стоит сохранить; создать директорию категории, сохранить паттерн в
    `PATTERNS/PATTERNS/<категория>/`.
  - **6.8.2.2** Записать строку в таблицу Deployments. Запись делаешь **сам** — вызываешь
    `owner_product_loop_record_deployment` (Deployments MCP, `:3215`). Поля: `platform=<ты>`,
    `model=<твой id модели>`, `tokens` (сколько израсходовал на этот шаг — честно; нет числа → 0),
    `commit_hash`, `step`, `page_url`, `status=ready`. `result` по умолчанию 3 — **звёзды ставит
    пользователь** позже в UI. «Код попал в продакшен» отмечаешь **только** после повторной проверки:
    страница отвечает HTTP 200.

### 6.9. Закрыть шаг
Перенести файл шага `<NN>-slug.md` из `NEW-STEPS/` в `DEVELOPMENT-STEPS/COMPLETED-STEPS/`
(`status:completed`, `completedAt`). В нём —
**максимально полный отчёт без сокращений**: что сделано, с чем столкнулись, какие паттерны использованы
и какие созданы, были ли ошибки развёртывания, какая модель работала, сколько токенов израсходовано.

### 6.10. Ingest в память
Выгрузить файл завершённого шага (из `COMPLETED-STEPS/`) в вектор-память (`POST /api/rag/ingest`,
заголовок `X-Agent-Identity`).

### 6.11. Отчёт архитектору
Доложить о завершении шага. Настоятельно рекомендовать:
