...
- 🔥 **РЕЖИМ `migration` НАЗВАН И ОПУБЛИКОВАН, НО НЕ ПОСТРОЕН** (2026-08-22, первый пункт очереди).
  ...
  **Что уже построено (533-1, 2026-08-22):** значение `migration` живёт в схеме и в типе
  `PLATFORM-CONFIG.developmentMode`, панель предлагает его четвёртой карточкой на странице «Режим
  разработки» — владелец может включить режим, и ты прочитаешь его на старте.

  **Чего физически нет:** вкладки «Переезд на Fractera» (где называется адрес чужого проекта), самого
  разбора чужого репозитория, дерева замысла, порождения шагов из декомпозиции, таблицы возможностей и
  переноса данных. Навыка `use-migration` тоже нет — он пишется ИЗ постройки, а не до неё.
