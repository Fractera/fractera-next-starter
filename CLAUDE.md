...
- **Как платформа меняется, когда правка действительно нужна** (делает архитектор): клон открытого
  репозитория к себе → правка и проверка вне боевого сервера → `git pull` в `/opt/fractera` и
  пересборка задетой службы. Второй среды у неё нет: упавшая сборка авторизации закрывает вход всем.
