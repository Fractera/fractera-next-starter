...
- 🔒 **НАВЫК «СНАЧАЛА ПОСМОТРИ, ЧТО У НАС УЖЕ ЕСТЬ» — И ОДНО МЕСТО ДЛЯ ИНСТРУМЕНТОВ**
  (требование владельца 2026-08-20, записано по живому провалу). Прежде чем строить что бы то ни
  было, агент обязан посмотреть список готовых инструментов. Сейчас такого навыка нет, и цена этого
  уже видна.

  **Что вскрылось.** Диалог перевода полей — готовый общий инструмент, и в его собственной шапке
  написано: «подключается любой сущностью с переводимыми полями». Живёт он в
  `components/i18n/translations-dialog.client.tsx` (плюс `translation-cell`, `use-translations`,
  словарь) — то есть **НЕ в `_tools/`** и **не в реестре инструментов панели**
  (`bridges/app/lib/tools-registry.ts`: там четыре — image-crop, video-trim, voice-input,
  code-view). Из-за этого кнопка «перевести» в разделах меню и подвала построена мимо него:
  зовёт `api/config/nav/translate` напрямую и показывает строчку текста вместо интерфейса,
  который для этого и делался.

  **Что должен закрыть будущий шаг:** одно место для инструментов, куда попадают ОБЕ стороны —
  панельная и приложенческая; переезд диалога переводов туда с записью в реестр; подключение
  кнопок перевода к нему вместо самодельного отказа; и сам навык, который заставляет смотреть в
  этот список ДО постройки. Пока навыка нет, любой следующий агент построит вторую копию — ровно
  так, как построил эту.
