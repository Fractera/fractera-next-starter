...
**Гейты.** Четырнадцать штук идут сами перед сборкой (`prebuild`) — маршруты, схемы, контент, секции,
диалоги, раскладка, типографика, защита, поиск, AIO, PWA, контраст, ссылки. Сами НЕ идут два, и звать
их надо руками: `npm run check:types` и `npm run check:i18n`. Собирать на Windows нельзя, поэтому эти
двое — единственное, что стоит между твоей правкой и сборкой на сервере.
