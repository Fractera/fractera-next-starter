...
|---|---|---|
| `use-platform-config` | десять выключателей: что читается, чего не решает, как увидеть перемену | ✅ |
...
- **Соцсети: факты пришли, навыка `use-app-config` ещё нет** (шаг 523, 2026-08-20). Записано, чтобы
  не потерять до того, как навык будет написан.

  **Два вопроса, и они в разных конфигах.** «Не хочу видеть соцсети в подвале» — вопрос НАЛИЧИЯ
  возможности, выключатель `socials` в `PLATFORM-CONFIG`. «Какие сети и какие профили» — содержание,
  `seo.socialLinks` в `APP-CONFIG`. Раньше выключателя не было вовсе: ряд появлялся оттого, что есть
  записи, и убрать его можно было только удалив данные.

  **Запись несёт ПРАВИЛО вместе со значением:** `{ id, name, urlTemplate, value, icon }`. Адрес
  считается (`https://t.me/{value}`), а не угадывается. Причина: правило у каждой сети своё —
  `wa.me/<номер>`, у LinkedIn личный профиль это `/in/`, а не `/company/`, у почты `mailto:`.
  🪦 Прежняя цепочка `if` держала правила ДВУМЯ копиями (`socialUrls` и подвал), и обе несли ошибку с
  LinkedIn. Старые четыре ключа читаются дословно, ВКЛЮЧАЯ эту ошибку: чинить правило задним числом
  нельзя — на работающих серверах лежит значение, собранное под него.

  **Значок кладётся в проект, а не берётся ссылкой на чужой хост** — страница обязана работать офлайн.
  В lucide брендовых знаков почти нет, свой `BrandX` нарисован руками именно поэтому.

  **Почему без модели не обойтись.** Свободное имя расходится: «телеграм», «Telegram», «tg» — одна
  сеть, три написания, и сопоставить их со значком и правилом нечем. Модель возвращает каноническое
  имя, слаг значка и шаблон ссылки; ответ ПРЕДЛАГАЕТСЯ, а не применяется.

  🔒 **Цикл полный, а не оцифровка названия** (требование владельца). Голосом приходит и сеть, и
  описание профиля («латиницей, транслитерацией, слова через дефис»). Конструктор строит кандидатов,
  ПРОВЕРЯЕТ их и спрашивает, какой имелся в виду.
  **У проверки ТРИ исхода, а не два:** есть · нет · сеть не отвечает посторонним. Instagram и LinkedIn
  закрываются от ботов независимо от того, существует профиль или нет, — и третий исход обязан
  называться третьим. Спрятать его в «нет» значит уверенно предложить не тот профиль.

  **Нет ключа OpenAI — честная деградация до ручного ввода**, а не отказ.
