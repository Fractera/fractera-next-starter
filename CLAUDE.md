...
| `use-dynamic-workflows` | ✍️ **писать** | единственный носитель процедуры «как владелец включает волны агентов и чем платит». Закон «работаешь один» уже стоит выше, в «Как строим», — навык несёт ровно процедуру включения и цену. Пишется хирургией `DYNAMIC-WORKFLOWS.md` (145 строк, никем не грузится): документ переезжает в навык и исчезает |
| `use-use-cases` | ✍️ **писать** | режим `cases` существует в `PLATFORM-CONFIG` и включается владельцем, а навыка к нему нет: сегодня агент в этом режиме читает про кейсы в четырёх разных местах инструкции. На него уже ссылаются таблица режимов и `use-products-config` — ссылка в никуда |
...
больше не искать. Два оставшихся плана (`use-dynamic-workflows`, `use-use-cases`) стоят в очереди на
письмо; `use-migration` ждёт живого прогона. Правило прежнее: **чего нет в таблице — того нет.**
