...
## Два слоя оформления: свой дизайн и кастомный

Языковое дерево расщеплено на два слоя (шаг 538). Группы в адресе не видны: `/ru/blog/x` остаётся
`/ru/blog/x`, где бы физически ни лежала папка.

| Слой | Что даёт странице | Кому |
|---|---|---|
| `app/[lang]/(designLayer)/` | меню, подвал, выдвижные панели, баннер согласия, тосты, установка приложения, сервис-воркер | **умолчание — сюда кладётся всё** |
| `app/[lang]/(freeLayer)/` | ничего: голый `<body>` и твоя разметка | кастомный дизайн, холсты во всю ширину |

**Что остаётся у обоих**, потому что иначе страница перестаёт быть документом: `<html lang>`,
метаданные, манифест, шрифт, перекрытие темы владельца и скрипт инициализации темы. Они живут в
`app/[lang]/layout.tsx` и ничего не рисуют — это словарь и метаданные, а не интерфейс.

🔒 **СВОБОДНЫЙ СЛОЙ ЗАКРЫТ, ПОКА ВЛАДЕЛЕЦ ЕГО НЕ ОТКРЫЛ.** Разрешение — `customDesign` в
`DESIGN-CONFIG` (панель → «Дизайн»), умолчание `false`. При `false` любая страница свободного слоя
отвечает 404: проверка стоит в его `layout.tsx`, а не в каждой странице, чтобы её нельзя было забыть.

**Что делать, когда владелец просит «свой дизайн»:**

1. Сначала спроси, нужен ли ему кастом ВООБЩЕ: блоками собирается почти всё, и они дешевле. Полный
   разбор — навык `use-design`.
2. Разрешение выключено — скажи это прямо и назови, где включается. Не обходи, не строй «пока в
   обычном слое, потом перенесём».
3. Разрешение есть — переноси **по одной странице**, а не весь сайт. Блог и каталог, оставшиеся в
   слое оформления, продолжают работать как работали.
4. Каждый перенос записывай ниже, в список. Строка без адреса — работа, о которой следующая сессия
   не узнает.

🔒 **Цена, которую называешь ВСЛУХ до переноса, а не после.** Страница в свободном слое: палитра из
панели её не перекрашивает, меню и подвала нет, согласия на cookie нет, установки приложения нет,
воркера нет. Дальше её ведёт архитектор руками — **это его зона ответственности, а не платформы**
(решение владельца 2026-08-22).

### Страницы на кастомном дизайне

_(пусто — ни одна страница не переведена)_

<!-- Форма записи: | адрес | когда переведена | что снято | кто ведёт |
     Строка живёт, пока страница в свободном слое; вернулась в оформление — строка удаляется. -->
...
- **Многоагентная работа.** Закон «работаешь один» записан выше, процедура — как владелец включает
  волны агентов и что при этом меняется — нет. `DYNAMIC-WORKFLOWS.md` (145 строк) документом по
  новому делению не является: это процедура, её место — навык `use-dynamic-workflows`. Закон
  действует во всех четырёх режимах и к кейсам не привязан. (`SINGLE-AGENT.md` удалён 2026-08-22:
  сам закон стоит выше, в разделе «Как строим», и там же оговорка — команда владельца считается
