...
Навык грузится по поводу, а не на старте. Существуют `manage-cases-and-steps` и `manage-app-settings`;
остальные пока имена-заготовки: `use-passport` · `use-auth` · `use-auth-providers` · `use-roles` ·
`use-panel` · `use-data` · `use-database` · `use-object-storage` · `use-vector-memory` · `use-map` ·
`use-channels` · `use-agentic-rag` · `use-app-config` · `use-platform-config` · `use-design-config` ·
`use-products-config` · `use-multi-lang`.
