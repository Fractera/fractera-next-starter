...
- `PRODUCTS-CONFIG` — продукты проекта, **по файлу-досье на продукт** → навык `use-products-config`

**Досье продукта** — `PRODUCTS-CONFIG/<id>.json`, один файл, в котором продукт есть весь: запись
(имя, структура, поверхность, адрес, публикация), вопросы и ответы приёма, кейсы с подтверждениями,
шаги разработки, план страниц, фаза и история переходов. Рядом `registry.json` раздаёт вечные `id` и
больше ничего, `<id>.quiz.jsonl` — стенограмма опроса, сырьё, а не состояние. Список продуктов — это
обход папки: файл-оглавление разошёлся бы с ней в первую неделю. Пишет досье панель, ты читаешь.
...
## Что уже построено — и почему это не удаляется

Свежий проект приезжает не пустым. Построенное — **образцы, с которых копируют**: каждый показывает,
как в Fractera делается свой класс вещей.

| Путь в `app/[lang]/` | Образец чего |
|---|---|
| `(publicLayer)/` | главная: секции из каталога, оболочка публичной страницы |
| `(publicLayer)/blog`, `(publicLayer)/blog/<slug>` | контент папкой на запись: SSG, языковые ячейки, разметка для машин |
| `(publicLayer)/products`, `(publicLayer)/products/[slug]` | контент из данных: динамический сегмент, статическая оболочка |
| `(publicLayer)/(footerPages)/{architecture,cookies,privacy,terms}` | правовые и служебные страницы одной группой |
...
🔒 **Удалять построенное запрещено.** Снеся образец, ты теряешь единственный источник паттерна и
дальше строишь по памяти — то есть мимо стандарта. Просьба «начать с чистого листа» удалением не
исполняется.

**Как исполняется.** Страница уходит из ВИДИМОСТИ, оставаясь в коде:

1. её путь — в `seo.disallowPaths` (`APP-CONFIG`), это `robots.txt`;
2. её адреса — вон из карты сайта;
3. её пункты — вон из верхнего и нижнего меню.

**Возврат — те же три шага назад**, и каждый проверяется: путь снова индексируется, адреса снова в
карте, пункт вернулся в меню, если владелец этого хочет.

🔒 **Сначала конфиг, потом код.** Убрать верхнее меню, погасить переключатель темы, сменить палитру,
спрятать страницы подвала — это значения в четырёх конфигах, и менять их не просто можно, а нужно
первым делом. Прежде чем писать код, спроси себя, решается ли задача переключателем; решается —
переключай и на этом заканчивай.
...
**Работаешь один.** Второго агента заводит только владелец своим словом; ни размер задачи, ни её
независимые части разрешения не дают. Попросил — запускаешь.
...
- **Контент: две модели.** Статическая — папка на запись, SSG, как у блога; динамическая — сегмент
  плюс данные, как у продуктов. Описано в `development-docs/CONTENT-ENGINE.md` §2 (584 строки,
  по-английски, никем не грузится) — не потеряно, но лежит не там, где ищут. Переезжает в навык.
