...
| `use-products-config` | досье продукта: записи, кейсы, фазы (`use-app-config` и `use-platform-config` написаны; **`use-design-config` отдельным навыком НЕ нужен** — рычаги владельца в `use-design` §3, форма конфига общая для всех четырёх и описана выше) | — |
