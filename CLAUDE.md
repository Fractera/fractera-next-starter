...
- **6.1.1** Обработать черновики на `/ai-draft-settings`: поочерёдно генерировать навыки/коннекторы (MCP),
...
- **6.2.1** Изучить `/development-steps`: новые шаги в `NEW-STEPS/`; часть шагов может быть не завершена.
- **6.2.2** Точечные запросы к вектор-памяти (`POST /api/rag/query`) для расширения контекста — по задаче,
  не дамп.
- **6.2.3** Беглый, поверхностный просмотр `/patterns`: запомнить **имена** существующих паттернов —
...
### 6.9. Закрыть шаг
Перенести файл шага в `DEVELOPMENT-STEPS/COMPLETED-STEPS/` (`status:completed`, `completedAt`). В нём —
...
Выгрузить completed-step в вектор-память (`POST /api/rag/ingest`, заголовок `X-Agent-Identity`).
