...
`HOW-USE-AUTH.md` (корень слота; рецепт + словарь ролей `lib/roles.ts`): форма доступа (public / private /
...
> Стандарт имени — **одно правило для MCP и навыков**: имя из 4–6 слов, цель — ближе к 6 ради человекочитаемости (имя читают и люди, не только агенты) (MCP вдобавок структурирован
...
> `ai-draft-settings.md` §«Naming convention». Оба ранних навыка приведены к правилу:
> `create-draft` → `propose-new-agent-skill-or-mcp`, `scaffold-route` →
> `scaffold-declared-route-into-component-skeleton`.
> При создании нового MCP/навыка действует **правило 19 dev-session**: синхронно
...
**гостевая авторизация** (`requiresGuestRegistration: true` + inline-guard `/api/me`, см. `HOW-USE-AUTH.md` §2). **Угадывать нельзя** —
неверный дефолт либо открывает приватную страницу, либо молча теряет работу посетителя. Правило —
`CLAUDE.md`/`AGENTS.md` §12b; рецепт — `HOW-USE-AUTH.md` (три формы доступа + гость→аккаунт). Без этого
...
> **Паритет с потоком A (шаг 126).** Поток B теперь материализован так же, как поток A
> (`/ai-draft-settings`): у `/architecture` есть **UI** (hover 🚀 Launch / 🗑 Delete на pending-узлах),
> самодостаточный **навык** `declare-architecture-page-or-task` и **MCP** `owner_arch_create_record` /
> `owner_arch_send_to_steps` (arch-bridge :3222). Launch собирает все pending-записи в **один** шаг и
> удаляет источники. Это **другой** цикл, чем эволюция навыка — сквозной разбор в документе-близнеце
> [`architecture-dev-pipeline.md`](./architecture-dev-pipeline.md) (ср. `skill-evolution-pipeline.md`).
...
> **⚠️ DEPRECATED (шаг 210).** Библиотека переиспользуемых паттернов (`PATTERNS/PATTERNS/`) и страница
> `/service/patterns` удалены: концепция переезжает в **слой «Дизайн»** (`fractera-design` :3004),
> который разрабатывается отдельно. Из §6–§7 действующим остаётся ТОЛЬКО механизм **антипаттернов**
> (`PATTERNS/ANTI-PATTERNS/` — обязательное чтение перед каждым деплоем, обязательная запись после
> падения деплоя). Остальной текст §6 сохранён как исторический референс для будущего слоя «Дизайн».
