...
6. **🆕 Мост ЗДОРОВ, но ОТСУТСТВУЕТ в `config.yaml` → Hermes не видит его инструменты (≠ шаг 92).**
   Это ДРУГОЙ корень, чем п.5: handshake работает, мост слушает порт, навык на месте — но в
   `/root/.hermes/config.yaml → mcp_servers` нет блока этого моста, поэтому Hermes **не подгружает его
   инструменты** при инициализации (он индексирует инструменты ТОЛЬКО из `config.yaml`, не сканирует порты).
   - **Сигнатура симптома (узнавать сразу):** Hermes **видит навык** (`skills_list`/`skill_view`), но
     соответствующего MCP-инструмента **нет в его наборе функций**; как fallback он читает данные напрямую
     (напр. `curl http://127.0.0.1:3300/...`) и/или пытается запустить скрипт сам — и **не может выполнить
     мутацию**. (Реальный кейс шага 147: `template-constructor-bridge :3224` отсутствовал в config.yaml →
     Hermes не видел `owner_template_*`, хотя `curl :3224/health`=ok и навык `compose-frozen-template` загружен.)
   - **Корень — гонка деплоя «Source → Vercel».** `config.yaml` генерится `bootstrap.sh` ОДИН раз при
     развёртывании. Если сервер бутстрапнулся РАНЬШЕ, чем Vercel разнёс обновлённый `bootstrap.sh` (с новым
     блоком моста), config.yaml содержит старый набор мостов. То же — если мост добавили в субстрат, но не
     перевыполнили bootstrap/не пересоздали config.yaml.
   - **Диагностика:** `curl 127.0.0.1:<port>/health` = ok (мост жив) **И** `grep <port> /root/.hermes/config.yaml`
     = пусто (моста нет в конфиге) → это п.6, не п.5.
   - **Решение (на живом сервере):** дописать блок моста в `config.yaml → mcp_servers` (зеркалить соседний:
     `<name>-bridge: { url: http://localhost:<port>, headers: { Authorization: "Bearer <HERMES_MCP_SECRET>" } }`)
     → `pm2 restart fractera-hermes` → проверить, что Hermes видит инструмент (`hermes mcp test` / повтор задачи).
   - **Профилактика (чтобы не повторять):** НИКОГДА не разворачивать новый сервер, пока Vercel не закончил
     деплой обновлённого `lib/bootstrap.sh` (правило «Source → Vercel»). Для будущих серверов исходник
     `bootstrap.sh` уже несёт блок моста — деплой после пропагации Vercel даёт верный config.yaml из коробки.
...
**Диагностика «Hermes видит навык, но не его MCP-инструмент» (не может выполнить мутацию, читает данные
через curl как fallback):** сравнить `curl 127.0.0.1:<port>/health` (мост жив?) с `grep <port>
/root/.hermes/config.yaml` (мост в конфиге?). Жив, но в конфиге нет → §4.6 (мост не попал в config.yaml,
гонка деплоя Source→Vercel): дописать блок в `config.yaml` + `pm2 restart fractera-hermes`.
