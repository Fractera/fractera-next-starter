...
export const PLATFORMS: { id: Platform; label: string; active: boolean; docsUrl: string; agentPrompt: string }[] = [
  ...
  { id: 'codex',       label: 'Codex',       active: false, docsUrl: 'https://developers.openai.com/codex/cli', agentPrompt: 'Install Codex CLI: read the documentation in docs/platforms/codex/ (start with AGENTS.md), then follow the install instructions exactly as written there. After successful install, verify with `codex --version` and set active: true for codex in app/app/@codeWorkspaceSlot/_components/coding-workspace/platforms.ts' },
  { id: 'gemini-cli',  label: 'Gemini CLI',  active: false, docsUrl: 'https://geminicli.com/docs/get-started/installation/', agentPrompt: 'Install Gemini CLI: read the documentation in docs/platforms/gemini-cli/ (start with AGENTS.md), then follow the install instructions exactly as written there. After successful install, verify with `gemini --version` and set active: true for gemini-cli in app/app/@codeWorkspaceSlot/_components/coding-workspace/platforms.ts' },
  { id: 'qwen-code',   label: 'Qwen Code',   active: false, docsUrl: 'https://qwen.ai/qwencode', agentPrompt: 'Install Qwen Code: read the documentation in docs/platforms/qwen-code/ (start with AGENTS.md), then follow the install instructions exactly as written there. After successful install, verify with `qwen --version` and set active: true for qwen-code in app/app/@codeWorkspaceSlot/_components/coding-workspace/platforms.ts' },
  ...
  { id: 'open-code',   label: 'Open Code',   active: false, docsUrl: 'https://opencode.ai/docs/en', agentPrompt: 'Install OpenCode: read the documentation in docs/platforms/open-code/ (start with AGENTS.md), then follow the install instructions exactly as written there. After successful install, verify with `opencode --version` and set active: true for open-code in app/app/@codeWorkspaceSlot/_components/coding-workspace/platforms.ts' },
];
