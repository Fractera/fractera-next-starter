...
export const ARCHITECT_GROUPS: readonly ArchitectGroup[] = [
  ...
  // 🪦 ГРУППЫ «РЕЖИМ РАЗРАБОТКИ» ЗДЕСЬ БОЛЬШЕ НЕТ (шаг 66, 2026-08-31) — она стала
  // СОБСТВЕННЫМ входом, ровно как «Дизайн» до неё. Решение владельца, дословно:
  // «перенесли дизайн в отдельную вкладку — вот также я хочу, чтобы перенесли шаги
  // разработки… главное освободить основную вкладку от избыточных и не связанных
  // инструментов».
  //
  // Она была ПЕРВОЙ группой со своим адресом (33-1) и потому единственной, кто
  // пользовался полем `route`. Поле оставлено: следующая переезжающая группа
  // пройдёт тем же путём, и заводить его заново было бы работой без нужды.
  //
  // Режим живёт на `/{lang}/architect/dev-mode` со своим меню из четырёх режимов
  // и своим входом в подвале сайта.
  // 🪦 ГРУППЫ «ДИЗАЙН» ЗДЕСЬ БОЛЬШЕ НЕТ (шаг 41, 2026-08-29) — она стала
  // СОБСТВЕННЫМ входом, а не одиннадцатой строкой этого меню. Указание владельца
  // отменило моё решение того же дня: «я хотел, чтобы здесь была ещё одна кнопка,
  // которая называется дизайн, и в этой кнопке были свои вкладки… чтобы они не были
  // в одной огромной вкладке настройки проекта, которая уже сильно перегружена».
  //
  // Оформление живёт на `/{lang}/architect/design` со своим входом в подвале
  // сайта и на ОБЩЕЙ раскладке `components/workspace/workspace-shell.tsx`.
  // 🪦 Своего меню у него нет с шага 49, а файл `_components/design-menu.tsx`,
  // который здесь назывался, удалён 2026-08-31 по слову владельца: он пережил
  // своего последнего потребителя на два шага.
] as const
