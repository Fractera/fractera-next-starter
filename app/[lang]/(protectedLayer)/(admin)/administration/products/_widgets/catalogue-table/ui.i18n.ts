// Слова таблицы администрирования — СВОИ (шаг 521, разбор 2026-08-21).
//
// 🔒 ЧТО ИЗМЕНИЛОСЬ ПРИ ПЕРЕЕЗДЕ, кроме владельца файла: ушли ключи `add` и
// `cancelAdd`. Администратор товары не заводит, и общий тулбар требовал от него
// передавать `add: ""` — пустую строку как способ спрятать кнопку. Право,
// описанное фальшивым значением, разойдётся с кодом молча; здесь кнопки нет
// физически, и прятать нечего.
//
...
const UI: Record<string, CatalogueTableUi> = {
...
export function catalogueTableUi(lang: string): CatalogueTableUi {
