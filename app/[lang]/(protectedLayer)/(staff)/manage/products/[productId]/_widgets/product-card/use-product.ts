...
// Поведение карточки: загрузка товара и сохранение отдельных полей.
//
// 🔒 ПЕРЕЕХАЛО В ВИДЖЕТ ИЗ `_lib/` (шаг 521, разбор 2026-08-21). Наверху оно
// лежало по привычке «поведение отдельно от вида», и у этой привычки был скрытый
// довод: рядом жил движок списка, общий на четыре слоя. Движок разобран по
// таблицам, а этот файл был единственным потребителем самого себя — карточка
// одна на весь проект. Вещь, у которой один хозяин, живёт у хозяина.
//
// Здесь же единственное место, где решается, ЧТО отправить на сервер при правке
// базового значения и при правке перевода: два разных запроса к одному маршруту,
// и путать их нельзя.
