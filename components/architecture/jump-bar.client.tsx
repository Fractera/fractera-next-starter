  ...
  return (
    <div className="rounded-xl border border-border bg-muted/20 p-3">
      <div className="mb-2 flex items-center gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="h-8 w-full rounded-md border border-border bg-background pl-7 pr-3 text-xs text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-1 focus:ring-ring"
          />
        </div>
        <button
          onClick={onReset}
      ...
      <div className="flex max-h-[280px] flex-wrap gap-1.5 overflow-y-auto pr-1">
        ...
        {shown.length === 0 && (
          <span className="px-1 py-1 text-[11px] text-muted-foreground/60">No match</span>
        )}
      </div>
    </div>
  )
