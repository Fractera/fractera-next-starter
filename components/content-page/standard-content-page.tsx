...
export function StandardContentPage({
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      {/* 🔒 ВЕРХНИЙ ОТСТУП КОЛОНКИ ЗАВИСИТ ОТ ТОГО, ЕСТЬ ЛИ ШАПКА СТРАНИЦЫ
          (владелец, 2026-08-17: «большое пространство воздуха под героем»).

          Шестьдесят четыре пикселя здесь отмерены под ШАПКУ — крошки, заголовок,
          подзаголовок, — и на странице с ней читаются правильно. У лендинга
          шапки нет: заголовок печатает первый экран (`titleInBody`), и эти
          64 пикселя оказывались чистой пустотой, вдобавок сложенной с нижним
          отступом самого первого экрана. Никто из нас их не считал: в коде
          секции их не видно, они приходят от шаблона.

          Значение — существующий токен `--page-py-work` (40px), тот же, что у
          рабочих экранов. Заводить третье число ради одного случая незачем.

          Нижний отступ не трогаем: он отделяет содержимое от подвала одинаково
          на всех страницах. */}
      <article
        data-app-column
        className="px-6 pb-[var(--page-py-content)]"
        style={{ paddingTop: `var(--page-py-${titleInBody ? "work" : "content"})` }}
      >
