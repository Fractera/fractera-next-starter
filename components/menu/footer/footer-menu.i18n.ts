...
// ─── Чат с ИИ-агентом (подвал) ──────────────────────────────────────────────
//
// 🔒 ЭТО НЕ РАЗДЕЛ СЛОЯ, А ОТДЕЛЬНАЯ СЛУЖБА на своём адресе (правка владельца
// 2026-09-02): кнопка стоит справа от Telegram-бота и ведёт наружу — на
// `chat.<домен>` или `:3600` по IP, ровно как вход в панель управления.
//
// 🔒 СЛОВАРЬ, А НЕ ТЕРНАР В РАЗМЕТКЕ. Инлайн `lang === "ru" ? … : …` запрещён в
// этом проекте: он расползается по файлам, и третий язык добавить становится
// нечем.
export type ChatLinkUi = { footer: string };

const CHAT_LINK: Record<string, ChatLinkUi> = {
  en: { footer: "AI agent chat" },
  ru: { footer: "Чат с ИИ-агентом" },
};

export function chatLinkUi(lang: string): ChatLinkUi {
  return CHAT_LINK[lang] ?? CHAT_LINK.en;
}
