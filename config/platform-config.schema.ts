...
export const platformConfigSchema = z.looseObject({
   ...
   *
   * 🛑 БЕЗ ЭТОЙ СТРОКИ КЛЮЧ ОТБРАСЫВАЕТСЯ ПРИ СОХРАНЕНИИ МОЛЧА: писатель проверяет
   * тело схемой, и незаявленное поле не доедет до диска, вернув при этом `200`.
   * Это третье из четырёх мест поля, и единственное, чей пропуск не даёт ни
   * ошибки сборки, ни сообщения.
   */
