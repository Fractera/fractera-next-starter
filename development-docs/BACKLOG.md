...
## 🚧 Telegram Desk: вложения принимаются как ТЕКСТ, файлы ещё нет (2026-08-23)

Тоннель разбирает и раскладывает **сообщение**. Фотография, видео, PDF и звуковой файл сегодня
доезжают до службы, но в продукт приходят только текстом подписи — самого файла в медиатеке не
появляется, `object_type` остаётся пустым, записи `media` в `tgdesk_artifacts` не рождается.

**Что придётся решить, и это НЕ одна работа, а три разного веса:**

| Род | Сложность | Суть |
|---|---|---|
| изображение | простая | забрать файл → медиатека по ИМЕНИ → сводка зрячей моделью («чек из Worten на 300 €») |
| звук | решена наполовину | голосовая заметка уже расшифровывается; присланный аудиофайл — тот же путь, но он длиннее и стоит дороже |
| видео и толстые PDF | **дорогая, и без спроса делать нельзя** | полная расшифровка часового видео и разбор слайдов — это минуты и заметные деньги |

🔒 **Тяжёлое не разбирается молча.** Владелец сказал прямо: на видео продукт обязан **спросить**,
нужна ли полная расшифровка со слайдами, и только потом тратить. Умолчание — сохранить файл, записать
подпись, разбор отложить.

**Проверить до постройки, а не после:** возможно, модель уже умеет часть этого нативно (описание
изображения одним вызовом, расшифровка длинного звука). Факт о чужом продукте берётся из его
документации, а не из памяти.

**Первый шаг, когда возьмёмся:** служба каналов сейчас отдаёт в дверь только `text`; нужен
`file_id` и род вложения, иначе приложению нечего забирать.
