# CANCELLED — what stopped being true, and what replaced it

**Nothing is deleted from the corpus; it is retired here.** This file exists for one reason: a law
removed without a trace is resurrected from memory by the next session — inaccurately, and as news.
It is read when somebody wonders why a rule they remember is gone, never at session start.

It left `CLAUDE.md` on 2026-08-23: a graveyard read by every session is paid for by every session.
