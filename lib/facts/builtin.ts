...
function materialFacts(): Fact[] {
  ...
  return Object.entries(known).map(([key, t]) => ({
    ...
    builtin: true,
    enabled: true,
    // ✍️ Рукописная половина раскрытия (81-9).
    example: key === "voice" ? "(голосовое сообщение на двадцать секунд)" : "вчера был у стоматолога",
    lost:
      key === "voice"
        ? "Сохраняется только расшифровка. Сама запись остаётся у Telegram, и всё, чем голос отличается от текста — интонация, паузы, оговорки, — теряется безвозвратно."
        : undefined,
  }))
}
