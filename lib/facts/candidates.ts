...
export async function noteFacets(facets: string[]): Promise<string> {
  ...
  // Реестр читается ОДИН раз на сообщение: имена и названия, приведённые к
  // нижнему регистру, — тег «Расходы» и признак «расходы» это одно и то же.
  const known = new Set<string>()
  for (const f of await allFacts()) {
    known.add(f.key.toLowerCase())
    known.add(f.key.split(".").pop()!.toLowerCase())
    known.add(f.title.toLowerCase())
  }
