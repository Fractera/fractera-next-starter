...
export function card(r: IngestResult): string {
  ...
  // 🔒 ОТВЕТ ПОКАЗЫВАЕТ ТОТ ЖЕ КОНВЕРТ, ЧТО УШЁЛ В ГРАФ (закон владельца
  // 2026-08-23). Иначе человек видит одно, а склад помнит другое — и связи,
  // которые продукт построил, для него не существуют.
  //
  // ✗ на картинку без чека продукт ответил «Сохранил, но разобрать не смог»,
  // хотя в графе к тому моменту лежал полный конверт с отправителем, датой и
  // признаками. Ответ был беднее собственной памяти.
  lines.push(r.summary ? `Записал: ${r.summary}` : "Записал.")
  ...
  // Конверт целиком — последним блоком, за чертой. Он длинный, и человеку
  // сперва нужен итог; но он ОБЯЗАН быть виден, потому что именно он лёг в
  // граф и именно по нему всё будет находиться.
  if (r.envelope) {
    lines.push("", "— в память ушло —", r.envelope)
  }
