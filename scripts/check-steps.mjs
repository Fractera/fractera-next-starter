...
//
// 🔒 ЧТО ИМЕННО ОН ЛОВИТ. Один класс дефекта: работа, которую ведут МИМО четырёх
...
// порядок и им не является: следующая сессия читает четыре адреса и не находит
...
// структуру самих четырёх адресов. Сторож, кричащий на законный код, отключают
...
// 1. Параллельный учёт рядом с четырьмя адресами.
...
// 2. Целостность самих четырёх адресов.
...
if (findings.length === 0) {
  console.log("  ✓ параллельного учёта рядом с четырьмя адресами нет")
...
for (const f of findings) {
  if (f.kind === "rival") console.log(`    ${f.rel} — параллельная система учёта рядом с четырьмя адресами`)
...
console.log(`
  Память этого проекта живёт по четырём адресам, и других не бывает:
  ...
  эти четыре адреса и не находит там ничего. Не хватает чего-то в них — скажите
  об этом владельцу, а не заводите пятый адрес.
